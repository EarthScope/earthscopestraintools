...
def pre_event_trend_correction(df, eq_time):
    ...
    for ch in df.columns:
        ...
        slope, intercept, r_value, p_value, std_err = stats.linregress(df_pre.index,
                                                                       df[ch][df.index < eq_time].interpolate())
        # print(slope)
...
def plot_coseismic_offset(df,
    ...
    if detrend:
        ...
        if detrend == 'linear':
            if eq_time:
                ...
                trend_c = pre_event_trend_correction(df - df.loc[df.first_valid_index()], eq_time)
                df -= trend_c
            else:
    ...
    if coseismic_offset:
        ...
        df_pre = df[(df.index > np.percentile(df.index, 0)) & (df.index <= np.percentile(df.index, 20))]
        df_post = df[(df.index > np.percentile(df.index, 20)) & (df.index <= np.percentile(df.index, 100))]
