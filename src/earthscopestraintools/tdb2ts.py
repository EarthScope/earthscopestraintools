...
from earthscopestraintools.tiledbtools import ProcessedStrainReader, RawStrainReader

logger = logging.getLogger(__name__)

def tiledb_raw_to_ts(
        reader: RawStrainReader,
        channels: list,
        units: str,
        start_ts: int = None,
        end_ts: int = None,
...
def tiledb_processed_to_ts(
        reader: ProcessedStrainReader,
        ...
        # print(df)
        data = reader.reindex_df(df=df, columns=data_types, attr="data")
        quality_df = reader.reindex_df(df=df, columns=data_types, attr="quality")
